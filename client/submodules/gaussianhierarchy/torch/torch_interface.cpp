...
{
	return Switching::forceSearch(
		child_boxes.size(0), 
		child_boxes.contiguous().data_ptr<float>(), 
		parent_boxes.contiguous().data_ptr<float>(),
		threshold,
		viewpoint.contiguous().data_ptr<float>(),
		viewdir.data_ptr<float>()[0], viewdir.data_ptr<float>()[1], viewdir.data_ptr<float>()[2],
		frustum_plans.contiguous().data_ptr<float>(), 
		leafs_tag.contiguous().data_ptr<bool>(), 
		if_culling, 
		// Output 
		last_frame.contiguous().data_ptr<int>(), 
		render_indices.contiguous().data_ptr<int>(),
		interpolation_weights.contiguous().data_ptr<float>());
}
...
// torch::Tensor& num_kids)
{
	...
	viewdir.data_ptr<float>()[0], viewdir.data_ptr<float>()[1], viewdir.data_ptr<float>()[2],
	ts.contiguous().data_ptr<float>(), 0);//, num_kids.contiguous().data_ptr<int>(), 0);
}