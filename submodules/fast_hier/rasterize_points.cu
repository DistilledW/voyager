	...
	const bool do_depth) // False
{
	...
	float boxptr[6] = {-1.3, -1.3, -1.3, 1.3, 1.3, 1.3};
	float elapse = 0;
	...
	std::tie(rendered, elapse_breakdown) = CudaRasterizer::Rasterizer::forward(
		...
		W, H,
		indices.contiguous().data_ptr<int>(), parent_indices.contiguous().data_ptr<int>(), // empty 
		...
		cov3D_precomp.contiguous().data_ptr<float>(), // empty

		viewmatrix.contiguous().data_ptr<float>(), projmatrix.contiguous().data_ptr<float>(),
	...
	return std::make_tuple(rendered, elapse, elapse_breakdown, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
...
VisDivergenceCUDA(
	const int pix_w, const int pix_h, const int batch_i,
	const torch::Tensor& background,
	const torch::Tensor& indices,
	const torch::Tensor& parent_indices,
	const torch::Tensor& ts,
	...
	const bool do_depth) // False
{
  ...
  torch::Tensor div_buffer = torch::full({256*256}, 0.0f, float_opts);
  torch::Tensor out_invdepth = torch::full({0, H, W}, 0.0, float_opts);
  if(do_depth)
  {
	// out_invdepth = torch::full({1, H, W}, 0.0, float_opts).contiguous();
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::divergence(
		pix_w, pix_h, batch_i,
	    geomFunc, binningFunc, imgFunc,
	    P, degree, M, // 点数，颜色的degree
		background.contiguous().data_ptr<float>(),
		...
		tan_fovx, tan_fovy,
		prefiltered, // False 
		div_buffer.contiguous().data_ptr<float>(),
		nullptr,
		radii.contiguous().data_ptr<int>(),
		rects.contiguous().data_ptr<int>(),
		nullptr, nullptr, debug);
		// &boxptr[0], &boxptr[3], debug);

  }
  return std::make_tuple(div_buffer);
}
