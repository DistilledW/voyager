...
// of Gaussians.
std::tuple<int, std::vector<float>> CudaRasterizer::Rasterizer::forward(
	...
	bool on_cpu)
{
	...
	if (*num_rendered == 0) 
		return std::make_tuple(0, elapse_breakdown);
...
// Forward rendering procedure for differentiable rasterization
// of Gaussians.
int CudaRasterizer::Rasterizer::divergence(
	const int pix_w, 
	const int pix_h, 
	const int batch_i,
	std::function<char* (size_t)> geometryBuffer,
	...
	bool on_cpu)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	const int2 pix_min = { pix_w, pix_h };
	CHECK_CUDA(FORWARD::divergence(
		pix_min, batch_i,
		imgState.ranges,
		binningState.point_list,
		width, height,
