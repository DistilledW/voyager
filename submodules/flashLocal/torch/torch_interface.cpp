...
LoadHierarchy(std::string filename) {
    ...
    int N = nodes.size();
    torch::TensorOptions intoptions =
    ...
    return std::make_tuple(
        pos_tensor, shs_tensor, alpha_tensor, scale_tensor, rot_tensor, nodes_tensor, box_tensor
    );
}

void WriteHierarchy(
    std::string filename, torch::Tensor &pos, torch::Tensor &shs, torch::Tensor &opacities,
    torch::Tensor &log_scales, torch::Tensor &rotations, torch::Tensor &nodes, torch::Tensor &boxes
) {
    ...
    writer.write(
        filename.c_str(), allP, allN, (Eigen::Vector3f *)pos.cpu().contiguous().data_ptr<float>(),
...
std::tuple<int, float> ExpandToSize(
    ...
    return Switching::expandToSize(
        ...
        render_indices.contiguous().data_ptr<int>(), nullptr, 
        nullptr, nullptr, mode 
    );
}

void ReorderNodes(torch::Tensor &nodes, torch::Tensor &boxes, bool grouped) {
    return dark::reorder_nodes(
        nodes.size(0), nodes.contiguous().data_ptr<int>(), boxes.contiguous().data_ptr<float>(),
        grouped
    );
}

void ReorderResult(torch::Tensor &nodes, torch::Tensor &nodes_for_render_indices) {
    return dark::reorder_result(
        nodes.size(0), nodes_for_render_indices.contiguous().data_ptr<int>()
    );
}

float GetTsIndexed(
    torch::Tensor &indices, float size, torch::Tensor &nodes, torch::Tensor &boxes,
    torch::Tensor &viewpoint, torch::Tensor &viewdir, torch::Tensor &ts, torch::Tensor &num_kids
) {
    return Switching::getTsIndexed(
        indices.size(0), indices.contiguous().data_ptr<int>(), size,
        nodes.contiguous().data_ptr<int>(), boxes.contiguous().data_ptr<float>(),
        viewpoint.data_ptr<float>()[0], viewpoint.data_ptr<float>()[1],
        viewpoint.data_ptr<float>()[2], viewdir.data_ptr<float>()[0], viewdir.data_ptr<float>()[1],
        viewdir.data_ptr<float>()[2], ts.contiguous().data_ptr<float>(),
