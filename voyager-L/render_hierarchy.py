...
def rotate_camera_roll(R, angle_deg):
    ...
    # 局部旋转，右乘
    return R @ Rz
def read_generate(args, n_frames = -1): # n帧 -> ((n-1)*n_frames + 1)帧
    ...
    camera_data = {} 
    settings = {} 
    settings["FoVx"] = 1.2781474513326148 
    settings["FoVy"] = 0.919927105183313 
    ...
    # T=np.array([-10, -12, 3]) 
    # R=rotate_camera_roll(R, -90) # 加是顺时针 
    ...
    #     } 
    with open("/workspace/code/dataset/camera_poses.json", "r")as f:
    ...
    for idx, (image_name, pose) in enumerate(camera_poses.items()):
        ...
        R_pre = R_cur 
        T_pre = T_cur 
    # with open(f"/workspace/code/dataset/camera_paths/Residence_like_bird/camera_poses.json", "w") as f: 
    #     json.dump(view_cameras, f, indent=4) 
    # with open(f"/workspace/code/dataset/camera_paths/Residence_like_bird/setting.json", "w") as f:
...
def render_set_accuracy(args, scene, pipe, out_dir, tau, eval):
    ...
    if args.compress:
        ...
        with torch.no_grad():
            _, _, _ = shs_vq(scene.gaussians.get_features[::16].view(-1, 48)) # initialize shs_vq 
    psnr_test = 0.0 
    ssims = 0.0
    lpipss = 0.0 
...
@torch.no_grad() 
def render_paths(args, scene, pipe, tau, eval):
    render_indices = torch.zeros(scene.gaussians._xyz.size(0)).int().cuda()
    # cameras = scene.getTestCameras() if eval else scene.getTrainCameras()
    camera_poses, settings = read_generate(scene, 15) 
    ...
    for idx, viewpoint in tqdm(camera_poses.items()): 
        ...
        image = torch.clamp(render_ret["render"], 0.0, 1.0) 
        file_path = os.path.join("/workspace/code/dataset/videos/Residence_like_bird/", f"{idx}.png") 
        try:
...
if __name__ == "__main__":
    ...
    for tau in args.taus:
        if args.path:
            render_paths(args, scene, pipe, tau, args.eval) 
        elif args.per:
            render_set_performance(args, scene, pipe, tau, args.eval) 
        else:
            render_set_accuracy(args, scene, pipe, os.path.join(args.out_dir, f"render_{tau}"), tau, args.eval) 